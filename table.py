...
class Table(QtGui.QTableView):
	...
	def addRow(self, song):
		attribs = song.getAttribsCustom(self.commPlaylist)
		nodes = [QStandardItem('')]
	...
	def __init__(self, parent, configOption):
		...
		self.commPlaylist = configOption['playlist_order']
	...
	def initUI(self):
		...
		self.addRow(Song({'FILE': '', 'LENGTH': '','CHANNELS': '', 'SAMPLERATE': '','BITRATE': ''}, self.commPlaylist))
		self.model().removeRow(0)

		#Remplis le titre des headers, avec des majuscules (title())
		headers = self.commPlaylist.title().replace('%','').split('|')
...
#----------PLAYER FUNCTIONS----------------------------------------
	def padd(self, uri):
		...
		self.playbin.set_property('uri', realUri)
...
#------------------------------------------------------------------------------------
	...
	def playSongFromTable(self):
		...
		print(index.row())
		song = index.model().itemFromIndex(index).data()
		self.play(song)
